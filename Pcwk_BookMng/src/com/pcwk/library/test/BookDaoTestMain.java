...
public class BookDaoTestMain {
	...
	public Book getInputData(Scanner scanner) {
		...
		String readInput = scanner.nextLine().trim();// 입력데이어터 받기!
		// System.out.println("readInput:"+readInput);

		String[] dataArr = readInput.split(",");// 8994492046-9788994492049,Java의 정석 기초편,남궁성,IT,1 배열을 자름
...
//		bookMain.saveFile();//파일에 기록하기:OK
		...
		do {
			...
			case "RE":// 전체조회
				...
				for (Book b : list) {
					System.out.println(b.getIsbn() + "," + b.getTitle() + "," + b.getAuthor() + "," + b.getGenre() + ","
							+ b.isAvailable());
				}
				break;
			...
			case "U":
				...
				if (bookMain.dao.isBookExists(book) != 1) {// 기존데이터가 있는가
					System.out.println("테이터가 존재 하지 않습니다..\n데이터를 확인 해주세요.");
					continue;
				}
			...
			case "R":// 단건 조회
				// 화면에서 도서번호를 입력 받는다.
				// BOOk객체 생성
				System.out.println("조회번호를 적어주세요.: 8994492046-9788994492049");
