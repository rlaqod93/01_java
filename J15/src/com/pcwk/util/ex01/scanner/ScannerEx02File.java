...
public class ScannerEx02File {
	...
	public static void main(String[] args) {
		...
		try {
			//절대 경로 방식으로 파일 읽기
//			Scanner scanner = new Scanner(new File("C:\\DCOM_20220127\\01_JAVA\\workspace\\J15\\src\\com\\pcwk\\util\\ex01\\scanner\\data99"));
			//상대 경로 방식으로 파일 읽기
			Scanner scanner = new Scanner(new File("data99.txt"));// /프로젝트명/data99.txt
			int sum = 0;//합계
			int cnt = 0;//읽은 라인수
			
			while(scanner.hasNextInt()) {//scanner.hasNextInt() Data가 있으면 true , 더이상 데이터가 없으면 false
			sum += scanner.nextInt();
			cnt++;	
			}
